...
from bkk_business_cycle.export import TABLE_FILES
from bkk_business_cycle.application import run_analysis, check_reference, export_tables
from bkk_business_cycle.domain import Estimated, EmploymentSplice, Variable
...
def test_all_publication_tables_exactly_match_r(
    results: AnalysisResult, tmp_path: Path
) -> None:
    assert check_reference(results, ROOT / "reference/tables").matches
    export_tables(results, tmp_path)
    for stem in TABLE_FILES.values():
        assert (tmp_path / f"{stem}.csv").read_bytes() == (
            ROOT / "reference/tables" / f"{stem}.csv"
        ).read_bytes()
...
def test_hp_banded_solver_against_dense_objective(size: int, smoothing: float) -> None:
    ...
    expected = values - np.linalg.solve(
        np.eye(size) + smoothing * difference.T @ difference, values
    )
    np.testing.assert_allclose(
        hp_cycle(values, smoothing), expected, rtol=0, atol=1e-11
    )
...
@pytest.mark.parametrize(
    "values", [[], [1.0, 2.0], [1.0, np.nan, 3.0], [1.0, np.inf, 3.0]]
)
...
def test_pairwise_missing_values_do_not_drop_other_pairs() -> None:
    ...
    assert pairwise_correlation(data, "a", "b") == Estimated(1.0, 2)
    assert pairwise_correlation(data, "a", "c") == Estimated(-1.0, 2)


@pytest.mark.parametrize(
    ("probability", "expected"),
    [(0, 1), (0.1, 1), (0.25, 1), (0.4, 2), (0.5, 2), (0.75, 3), (1, 4)],
)
...
def test_splice_switch_factor_and_original_full_join_order() -> None:
    ...
    oecd = pl.DataFrame(
        {
            "time": ["2011-Q2", "2011-Q4", "2012-Q1", "2015-Q2"],
            "value": [40.0, 50.0, 55.0, 60.0],
        }
    )
    result = splice_employment_series(fred, oecd, EmploymentSplice("GBR"))
...
def test_france_truncation_and_labor_country_coverage(results: AnalysisResult) -> None:
    france = results.series[Variable.EMPLOYMENT].filter(pl.col("location") == "FRA")
    assert france["time"].max() == "2013-Q4"
    assert not set(results.series[Variable.EMPLOYMENT]["location"]) & {
        "CHE",
        "EA19",
        "EU15",
    }
    assert (
        results.panel.select(
            pl.struct("variable", "location", "time").is_duplicated().any()
        ).item()
        is False
    )
